...
import { useLiveMarketStore } from "@/lib/store/store";

export function StockTicker() {
  const { quotes } = useLiveMarketStore();
  ...
  const tickerItems = [
    ...INDICES.map((idx) => {
      ...
      return {
        ...
        price: (live?.price ?? idx.value).toLocaleString("en-IN"),
        change: live?.changePercent ?? idx.changePercent,
        isIndex: true,
      };
    }),
    ...STOCKS.slice(0, 6).map((s) => {
      ...
      return {
        symbol: s.symbol,
        price: `₹${(live?.price ?? s.price).toLocaleString("en-IN")}`,
        change: live?.changePercent ?? s.changePercent,
        isIndex: false,
      };
    }),
  ];
  ...
  return (
    <div className="w-full overflow-hidden bg-bg-secondary border-b border-border ticker-fade-left py-2">
      <div className="flex animate-ticker-scroll whitespace-nowrap gap-0">
        {items.map((item, i) => (
          <div key={i} className="inline-flex items-center gap-2 px-6 border-r border-border/50">
            ...
            <span className={`inline-flex items-center gap-0.5 text-xs font-num font-medium ${
